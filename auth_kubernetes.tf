...
resource "vault_kubernetes_auth_backend_role" "this" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "Роль ${each.key}: token_period задан вместе с ненулевым token_max_ttl или token_explicit_max_ttl — потолок прижмёт токен, сколько его ни продлевай. Для периодического токена оба поля должны быть 0."
    }
  }
}
